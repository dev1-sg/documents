...
def get_latest_image(client, repo_name):
    try:
        ...
        def sort_key(img):
            return img.get("imagePushedAt", datetime.min)

        valid_images = [img for img in images if img.get("imageTags")]
        latest = max(valid_images or images, key=sort_key)
        ...
        if pushed_at:
            pushed_at = pushed_at.astimezone(TZ).isoformat()
        else:
            pushed_at = None
...
def lambda_handler(event, context):
    ...
    return {
        ...
        "body": json.dumps({"docker-${image_group}-images": images}, indent=2)
    }
